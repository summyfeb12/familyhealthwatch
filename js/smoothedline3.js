function drawlinegraph3(datum, p1, p2, curveflag, curvetype, hoverflag, svgname, areaflag, xwidth, yheight, param, unit, linecolor, areafill, dotfill){
    ...
    if((svgname=="#walk-graph3")|(svgname=="#calorie-graph3")){
        count = 22;
        var den = 4;
    }
    else if(svgname=="#sleep-graph3")
        var den = 8;
	else
		var den = 4;
    
    datum.forEach(function(d){
      if((count%den == 3)&(count<50)){
		  if(svgname!="#calorie-graph3"){
    ...
    else if((svgname!="#walk-graph3")&(svgname!="#calorie-graph3")){
        svg.selectAll("dot")
            .data(datum)
            .enter().append("circle")
            .attr("r", 4)
...
function drawwaterfall3(datum, svgname, xwidth, yheight, linecolor, areafill){
    // set the dimensions and margins of the graph
    var margin = {top: 20, right: 20, bottom: 30, left: 50},
        width = xwidth,
        height = yheight;
