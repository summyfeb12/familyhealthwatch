function drawlinegraph1(datum, p1, p2, curveflag, curvetype, hoverflag, svgname, areaflag, xwidth, yheight, param, unit, linecolor, areafill, dotfill){
    ...
    if((svgname=="#walk-graph1")|(svgname=="#calorie-graph1")){
        count = 36;
        var den = 2;
    }
    else if(svgname=="#sleep-graph1")
        var den = 8;
	else
		var den = 3;
    
    datum.forEach(function(d){
      if((count%den == 1)&(count<50)){
		  if(svgname!="#calorie-graph1"){
    ...
    else if((svgname!="#walk-graph1")&(svgname!="#calorie-graph1")){
        svg.selectAll("dot")
            .data(datum)
            .enter().append("circle")
            .attr("r", 4)
...
function drawwaterfall1(datum, svgname, xwidth, yheight, linecolor, areafill){
    // set the dimensions and margins of the graph
    var margin = {top: 20, right: 20, bottom: 30, left: 50},
        width = xwidth,
        height = yheight;
